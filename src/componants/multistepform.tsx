...
const MultiStepForm: React.FC = () => {
      ...
      for (const key in values) {
        if (Object.prototype.hasOwnProperty.call(values, key)) {
          ...
          if (
            ...
            key.startsWith("rel") ||
            key.startsWith("caste") ||
            key.startsWith("gend") ||
            key.startsWith("disab") ||
            key.startsWith("marit") ||
            key.startsWith("id_")
          ) {
            stepName = "Personal_details";
          } else if (
            key.startsWith("cno") ||
            key.startsWith("emerg") ||
            key.startsWith("email") ||
            key.startsWith("address")
          ) {
            stepName = "Contact_details";
          } else if (
            key.startsWith("tenth") ||
            key.startsWith("twelfth") ||
            key.startsWith("ug_") ||
            key.startsWith("grad_")
          ) {
            stepName = "Educational_details";
          } else if (
            key.startsWith("behav") ||
            key.startsWith("institute") ||
            key.startsWith("doj") ||
            key.startsWith("dol") ||
            key.startsWith("post") ||
            ...
            key.startsWith("availableLeaves")
          ) {
          ...
          if (value) {
            if (stepName === "Personal_details") {
              stepData.Personal_details[key as keyof PersonalDetailsValues] =
                value;
            } else if (stepName === "Contact_details") {
              stepData.Contact_details[key as keyof ContactDetailsValues] =
                value;
            } else if (stepName === "Educational_details") {
              stepData.Educational_details[
                key as keyof EducationalDetailsValues
              ] = value;
            } else if (stepName === "Prev_postings") {
              stepData.Prev_postings[key as keyof DynamicInstituteFormValues] =
                value;
            }
          }
        }
  ...
  return (
    <div>
      ...
      <div className="w-full p-8 bg-indigo-50/50 border rounded-lg">
      ...
      <div className="flex justify-between mt-6">
        {currentStep > 0 && (
          <button
            onClick={back}
            className="bg-gray-500 text-white font-bold rounded px-4 py-2 w-1/4"
          >
            Back
          </button>
        )}
        {currentStep < steps.length - 1 && (
          <button
            onClick={next}
            className="bg-indigo-500 text-white font-bold rounded px-4 py-2 w-1/4"
          >
            Next
          </button>
        )}
        {currentStep === steps.length - 1 && (
          <button
            onClick={submit}
            className="bg-green-500 text-white font-bold rounded px-4 py-2 w-1/4"
          >
            Submit
          </button>
        )}
